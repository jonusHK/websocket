...
Revision ID: 8653c2eab052
Revises: 
Create Date: 2023-03-15 02:47:20.541219
...
import server
from server.core.enums import ChatHistoryType, ProfileImageType, RelationshipType, ChatRoomType

# revision identifiers, used by Alembic.
revision = '8653c2eab052'
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    sa.Column('updated', sa.DateTime(timezone=True), nullable=False),
    sa.Column('id', sa.BigInteger(), nullable=False),
    sa.Column('redis_id', sa.String(length=32), nullable=False),
    sa.Column('room_id', sa.BigInteger(), nullable=False),
    sa.Column('user_profile_id', sa.BigInteger(), nullable=True),
    sa.Column('contents', sa.Text(), nullable=True),
    sa.Column('type', server.core.utils.IntTypeEnum(enum_class=ChatHistoryType), nullable=False),
    sa.Column('is_active', sa.Boolean(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['user_profile_id'], ['user_profiles.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('room_id', 'user_profile_id', name='unique association for both room_id and user_profile_id.')
    )
    op.create_index(op.f('ix_chat_room_user_association_created'), 'chat_room_user_association', ['created'], unique=False)
    op.create_index(op.f('ix_chat_room_user_association_room_id'), 'chat_room_user_association', ['room_id'], unique=False)
    op.create_index(op.f('ix_chat_room_user_association_user_profile_id'), 'chat_room_user_association', ['user_profile_id'], unique=False)
    ...
    sa.ForeignKeyConstraint(['user_profile_id'], ['user_profiles.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('history_id', 'user_profile_id', name='unique association for both history_id and user_profile_id.')
    )
    op.create_index(op.f('ix_chat_history_user_association_history_id'), 'chat_history_user_association', ['history_id'], unique=False)
    op.create_index(op.f('ix_chat_history_user_association_user_profile_id'), 'chat_history_user_association', ['user_profile_id'], unique=False)
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_chat_history_user_association_user_profile_id'), table_name='chat_history_user_association')
    op.drop_index(op.f('ix_chat_history_user_association_history_id'), table_name='chat_history_user_association')
    ...
    op.drop_index(op.f('ix_chat_room_user_association_user_profile_id'), table_name='chat_room_user_association')
    op.drop_index(op.f('ix_chat_room_user_association_room_id'), table_name='chat_room_user_association')
